...
project_name <- "marjan_2012_experimental_small"
...
summarize_each_file <- function(fname) {
  ...
  df %>% 
    ...
    mutate(nobs = nobs,
           ...
           selection_policy = factor(researcher_parameters_research_strategy_initial_decision_policies_0_0),
           tmean = experiment_parameters_data_strategy_measurements_means_5,
           effect = effect) %>%
    mutate(eff_abs_diff= effect - tmean) %>%
    group_by(tmean, covs, tnobs, research_strategy, is_hacked, selection_policy) %>%
    summarize(sigmean = mean(sig),
              mean_nobs = mean(nobs),
              mean_eff = mean(effect),
              mean_eff_diff = mean(eff_abs_diff)) %>%
              mutate(size = "Small") -> agg_df
